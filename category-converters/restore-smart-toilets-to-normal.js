...
const restoreSMARTTOILETSToNormal = async () => {
  try {
    console.log('\n🔄 RESTORING SMART TOILETS TO NORMAL PRICING...\n');
    ...
    console.log(`   Total SMART TOILETS Products: ${restoredProducts.length}`);
    ...
    if (priceStats.length > 0) {
      ...
      console.log(`   Regular SMART TOILETS with Price: ${stats.count}`);
    ...
    console.log('\n🎉 SMART TOILETS RESTORATION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error restoring smart-toilets to normal pricing:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await restoreSMARTTOILETSToNormal();
