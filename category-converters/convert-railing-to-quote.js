...
const convertRAILINGToQuote = async () => {
  try {
    console.log('\n🔄 CONVERTING RAILING TO QUOTE MODE...\n');
    
    // Find railing category
    const category = await Category.findOne({ 
      $or: [
        { name: { $regex: /railing/i } },
        { slug: { $regex: /railing/i } }
      ]
    });
    
    if (!category) {
      console.log('❌ RAILING category not found');
      return;
    }
    
    console.log(`📂 Working with RAILING Category: ${category.name} (${category._id})`);
    ...
    console.log(`📦 Found ${products.length} railing products to convert`);
    
    if (products.length === 0) {
      console.log('✅ No railing products to convert');
      return;
    }
    ...
    console.log(`   Total RAILING Products: ${convertedProducts.length}`);
    ...
    console.log('\n🎉 RAILING CONVERSION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error converting railing to quote mode:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await convertRAILINGToQuote();
