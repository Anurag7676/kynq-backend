...
const convertPATTERNTILESToQuote = async () => {
  try {
    console.log('\n🔄 CONVERTING PATTERN TILES TO QUOTE MODE...\n');
    
    // Find pattern-tiles category
    const category = await Category.findOne({ 
      $or: [
        { name: { $regex: /pattern-tiles/i } },
        { slug: { $regex: /pattern-tiles/i } }
      ]
    });
    
    if (!category) {
      console.log('❌ PATTERN TILES category not found');
      return;
    }
    
    console.log(`📂 Working with PATTERN TILES Category: ${category.name} (${category._id})`);
    ...
    console.log(`📦 Found ${products.length} pattern-tiles products to convert`);
    
    if (products.length === 0) {
      console.log('✅ No pattern-tiles products to convert');
      return;
    }
    ...
    console.log(`   Total PATTERN TILES Products: ${convertedProducts.length}`);
    ...
    console.log('\n🎉 PATTERN TILES CONVERSION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error converting pattern-tiles to quote mode:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await convertPATTERNTILESToQuote();
