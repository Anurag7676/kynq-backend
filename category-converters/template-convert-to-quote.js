...
const convertCATEGORYToQuote = async () => {
  try {
    console.log('\n🔄 CONVERTING CATEGORY TO QUOTE MODE...\n');
    ...
    if (products.length === 0) {
      console.log('✅ No products to convert');
      return;
    }
    ...
    console.error('❌ Error converting to quote mode:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await convertCATEGORYToQuote();
