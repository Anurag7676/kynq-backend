...
const convertVANITIESToQuote = async () => {
  try {
    console.log('\n🔄 CONVERTING VANITIES TO QUOTE MODE...\n');
    ...
    console.log(`📂 Working with VANITIES Category: ${category.name} (${category._id})`);
    ...
    console.log(`📦 Found ${products.length} vanities products to convert`);
    
    if (products.length === 0) {
      console.log('✅ No vanities products to convert');
      return;
    }
    ...
    console.log(`   Total VANITIES Products: ${convertedProducts.length}`);
    ...
    console.log('\n🎉 VANITIES CONVERSION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error converting vanities to quote mode:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await convertVANITIESToQuote();
