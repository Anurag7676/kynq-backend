...
const restoreRAILINGToNormal = async () => {
  try {
    console.log('\n🔄 RESTORING RAILING TO NORMAL PRICING...\n');
    ...
    console.log(`   Total RAILING Products: ${restoredProducts.length}`);
    ...
    if (priceStats.length > 0) {
      ...
      console.log(`   Regular RAILING with Price: ${stats.count}`);
    ...
    console.log('\n🎉 RAILING RESTORATION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error restoring railing to normal pricing:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await restoreRAILINGToNormal();
