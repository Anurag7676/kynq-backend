...
const restoreWINDOWSToNormal = async () => {
  try {
    console.log('\n🔄 RESTORING WINDOWS TO NORMAL PRICING...\n');
    ...
    console.log(`   Total WINDOWS Products: ${restoredProducts.length}`);
    ...
    if (priceStats.length > 0) {
      ...
      console.log(`   Regular WINDOWS with Price: ${stats.count}`);
    ...
    console.log('\n🎉 WINDOWS RESTORATION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error restoring windows to normal pricing:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await restoreWINDOWSToNormal();
