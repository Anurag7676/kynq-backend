...
const convertDECORATIVELIGHTSToQuote = async () => {
  try {
    console.log('\n🔄 CONVERTING DECORATIVE LIGHTS TO QUOTE MODE...\n');
    
    // Find decorative-lights category
    const category = await Category.findOne({ 
      $or: [
        { name: { $regex: /decorative-lights/i } },
        { slug: { $regex: /decorative-lights/i } }
      ]
    });
    
    if (!category) {
      console.log('❌ DECORATIVE LIGHTS category not found');
      return;
    }
    
    console.log(`📂 Working with DECORATIVE LIGHTS Category: ${category.name} (${category._id})`);
    ...
    console.log(`📦 Found ${products.length} decorative-lights products to convert`);
    
    if (products.length === 0) {
      console.log('✅ No decorative-lights products to convert');
      return;
    }
    ...
    console.log(`   Total DECORATIVE LIGHTS Products: ${convertedProducts.length}`);
    ...
    console.log('\n🎉 DECORATIVE LIGHTS CONVERSION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error converting decorative-lights to quote mode:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await convertDECORATIVELIGHTSToQuote();
