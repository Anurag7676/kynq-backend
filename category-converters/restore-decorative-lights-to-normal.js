...
const restoreDECORATIVELIGHTSToNormal = async () => {
  try {
    console.log('\n🔄 RESTORING DECORATIVE LIGHTS TO NORMAL PRICING...\n');
    ...
    console.log(`   Total DECORATIVE LIGHTS Products: ${restoredProducts.length}`);
    ...
    if (priceStats.length > 0) {
      ...
      console.log(`   Regular DECORATIVE LIGHTS with Price: ${stats.count}`);
    ...
    console.log('\n🎉 DECORATIVE LIGHTS RESTORATION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error restoring decorative-lights to normal pricing:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await restoreDECORATIVELIGHTSToNormal();
