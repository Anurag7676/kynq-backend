...
const restoreSHOWERCUBICLESToNormal = async () => {
  try {
    console.log('\n🔄 RESTORING SHOWER CUBICLES TO NORMAL PRICING...\n');
    ...
    console.log(`   Total SHOWER CUBICLES Products: ${restoredProducts.length}`);
    ...
    if (priceStats.length > 0) {
      ...
      console.log(`   Regular SHOWER CUBICLES with Price: ${stats.count}`);
    ...
    console.log('\n🎉 SHOWER CUBICLES RESTORATION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error restoring shower-cubicles to normal pricing:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await restoreSHOWERCUBICLESToNormal();
