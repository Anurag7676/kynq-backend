...
const restoreJACUZZIToNormal = async () => {
  try {
    console.log('\n🔄 RESTORING JACUZZI TO NORMAL PRICING...\n');
    ...
    console.log(`   Total JACUZZI Products: ${restoredProducts.length}`);
    ...
    if (priceStats.length > 0) {
      ...
      console.log(`   Regular JACUZZI with Price: ${stats.count}`);
    ...
    console.log('\n🎉 JACUZZI RESTORATION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error restoring jacuzzi to normal pricing:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await restoreJACUZZIToNormal();
