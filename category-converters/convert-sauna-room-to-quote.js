...
const convertSAUNAROOMToQuote = async () => {
  try {
    console.log('\n🔄 CONVERTING SAUNA ROOM TO QUOTE MODE...\n');
    
    // Find sauna-room category
    const category = await Category.findOne({ 
      $or: [
        { name: { $regex: /sauna-room/i } },
        { slug: { $regex: /sauna-room/i } }
      ]
    });
    
    if (!category) {
      console.log('❌ SAUNA ROOM category not found');
      return;
    }
    
    console.log(`📂 Working with SAUNA ROOM Category: ${category.name} (${category._id})`);
    ...
    console.log(`📦 Found ${products.length} sauna-room products to convert`);
    
    if (products.length === 0) {
      console.log('✅ No sauna-room products to convert');
      return;
    }
    ...
    console.log(`   Total SAUNA ROOM Products: ${convertedProducts.length}`);
    ...
    console.log('\n🎉 SAUNA ROOM CONVERSION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error converting sauna-room to quote mode:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await convertSAUNAROOMToQuote();
