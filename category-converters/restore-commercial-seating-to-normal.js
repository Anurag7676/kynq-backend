...
const restoreCOMMERCIALSEATINGToNormal = async () => {
  try {
    console.log('\n🔄 RESTORING COMMERCIAL SEATING TO NORMAL PRICING...\n');
    ...
    console.log(`   Total COMMERCIAL SEATING Products: ${restoredProducts.length}`);
    ...
    if (priceStats.length > 0) {
      ...
      console.log(`   Regular COMMERCIAL SEATING with Price: ${stats.count}`);
    ...
    console.log('\n🎉 COMMERCIAL SEATING RESTORATION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error restoring commercial-seating to normal pricing:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await restoreCOMMERCIALSEATINGToNormal();
