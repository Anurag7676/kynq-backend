...
const restoreCATEGORYToNormal = async () => {
  try {
    console.log('\n🔄 RESTORING CATEGORY TO NORMAL PRICING...\n');
    
    // Find category
    const category = await Category.findOne({ 
      $or: [
        { name: { $regex: /CATEGORY/i } },
        { slug: { $regex: /CATEGORY/i } }
      ]
    });
    ...
    console.log(`📂 Working with Category: ${category.name} (${category._id})`);
    ...
    console.log(`   Total Products: ${restoredProducts.length}`);
    ...
    if (priceStats.length > 0) {
      ...
      console.log(`   Regular Products with Price: ${stats.count}`);
    ...
    console.error('❌ Error restoring to normal pricing:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await restoreCATEGORYToNormal();
