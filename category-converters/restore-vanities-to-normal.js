...
const restoreVANITIESToNormal = async () => {
  try {
    console.log('\n🔄 RESTORING VANITIES TO NORMAL PRICING...\n');
    ...
    console.log(`   Total VANITIES Products: ${restoredProducts.length}`);
    ...
    if (priceStats.length > 0) {
      ...
      console.log(`   Regular VANITIES with Price: ${stats.count}`);
    ...
    console.log('\n🎉 VANITIES RESTORATION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error restoring vanities to normal pricing:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await restoreVANITIESToNormal();
