...
const convertWINDOWSToQuote = async () => {
  try {
    console.log('\n🔄 CONVERTING WINDOWS TO QUOTE MODE...\n');
    
    // Find windows category
    const category = await Category.findOne({ 
      $or: [
        { name: { $regex: /windows/i } },
        { slug: { $regex: /windows/i } }
      ]
    });
    
    if (!category) {
      console.log('❌ WINDOWS category not found');
      return;
    }
    
    console.log(`📂 Working with WINDOWS Category: ${category.name} (${category._id})`);
    ...
    console.log(`📦 Found ${products.length} windows products to convert`);
    
    if (products.length === 0) {
      console.log('✅ No windows products to convert');
      return;
    }
    ...
    console.log(`   Total WINDOWS Products: ${convertedProducts.length}`);
    ...
    console.log('\n🎉 WINDOWS CONVERSION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error converting windows to quote mode:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await convertWINDOWSToQuote();
