...
const restoreWALLCOVERINGToNormal = async () => {
  try {
    console.log('\n🔄 RESTORING WALL COVERING TO NORMAL PRICING...\n');
    ...
    console.log(`   Total WALL COVERING Products: ${restoredProducts.length}`);
    ...
    if (priceStats.length > 0) {
      ...
      console.log(`   Regular WALL COVERING with Price: ${stats.count}`);
    ...
    console.log('\n🎉 WALL COVERING RESTORATION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error restoring wall-covering to normal pricing:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await restoreWALLCOVERINGToNormal();
