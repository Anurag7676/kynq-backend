...
const convertIndoorFurnitureToQuote = async () => {
  try {
    console.log('\n🔄 CONVERTING INDOOR FURNITURE TO QUOTE MODE...\n');
    
    // Find indoor-furniture category
    const category = await Category.findOne({ 
      $or: [
        { name: { $regex: /indoor-furniture/i } },
        { slug: { $regex: /indoor-furniture/i } }
      ]
    });
    
    if (!category) {
      console.log('❌ Indoor Furniture category not found');
      return;
    }
    
    console.log(`📂 Working with Indoor Furniture Category: ${category.name} (${category._id})`);
    ...
    console.log(`📦 Found ${products.length} indoor-furniture products to convert`);
    
    if (products.length === 0) {
      console.log('✅ No indoor-furniture products to convert');
      return;
    }
    ...
    console.log(`   Total Indoor Furniture Products: ${convertedProducts.length}`);
    ...
    console.log('\n🎉 INDOOR FURNITURE CONVERSION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error converting indoor-furniture to quote mode:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await convertIndoorFurnitureToQuote();
