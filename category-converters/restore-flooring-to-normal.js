...
const restoreFLOORINGToNormal = async () => {
  try {
    console.log('\n🔄 RESTORING FLOORING TO NORMAL PRICING...\n');
    ...
    console.log(`   Total FLOORING Products: ${restoredProducts.length}`);
    ...
    if (priceStats.length > 0) {
      ...
      console.log(`   Regular FLOORING with Price: ${stats.count}`);
    ...
    console.log('\n🎉 FLOORING RESTORATION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error restoring flooring to normal pricing:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await restoreFLOORINGToNormal();
