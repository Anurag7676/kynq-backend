...
const convertFABRICSToQuote = async () => {
  try {
    console.log('\n🔄 CONVERTING FABRICS TO QUOTE MODE...\n');
    
    // Find fabrics category
    const category = await Category.findOne({ 
      $or: [
        { name: { $regex: /fabrics/i } },
        { slug: { $regex: /fabrics/i } }
      ]
    });
    
    if (!category) {
      console.log('❌ FABRICS category not found');
      return;
    }
    
    console.log(`📂 Working with FABRICS Category: ${category.name} (${category._id})`);
    ...
    console.log(`📦 Found ${products.length} fabrics products to convert`);
    
    if (products.length === 0) {
      console.log('✅ No fabrics products to convert');
      return;
    }
    ...
    console.log(`   Total FABRICS Products: ${convertedProducts.length}`);
    ...
    console.log('\n🎉 FABRICS CONVERSION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error converting fabrics to quote mode:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await convertFABRICSToQuote();
