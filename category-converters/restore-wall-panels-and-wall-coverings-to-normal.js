...
const restoreWallPanelsAndWallCoveringsToNormal = async () => {
  try {
    console.log('\n🔄 RESTORING WALL PANELS & WALL COVERINGS TO NORMAL PRICING...\n');
    ...
    console.log(`   Total WALL PANELS & WALL COVERINGS Products: ${restoredProducts.length}`);
    ...
    if (priceStats.length > 0) {
      ...
      console.log(`   Regular WALL PANELS & WALL COVERINGS with Price: ${stats.count}`);
    ...
    console.log('\n🎉 WALL PANELS & WALL COVERINGS RESTORATION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error restoring wall-panels-and-wall-coverings to normal pricing:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await restoreWallPanelsAndWallCoveringsToNormal();
