...
const convertSHOWERCUBICLESToQuote = async () => {
  try {
    console.log('\n🔄 CONVERTING SHOWER CUBICLES TO QUOTE MODE...\n');
    
    // Find shower-cubicles category
    const category = await Category.findOne({ 
      $or: [
        { name: { $regex: /shower-cubicles/i } },
        { slug: { $regex: /shower-cubicles/i } }
      ]
    });
    
    if (!category) {
      console.log('❌ SHOWER CUBICLES category not found');
      return;
    }
    
    console.log(`📂 Working with SHOWER CUBICLES Category: ${category.name} (${category._id})`);
    ...
    console.log(`📦 Found ${products.length} shower-cubicles products to convert`);
    
    if (products.length === 0) {
      console.log('✅ No shower-cubicles products to convert');
      return;
    }
    ...
    console.log(`   Total SHOWER CUBICLES Products: ${convertedProducts.length}`);
    ...
    console.log('\n🎉 SHOWER CUBICLES CONVERSION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error converting shower-cubicles to quote mode:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await convertSHOWERCUBICLESToQuote();
