...
const convertRUGSToQuote = async () => {
  try {
    console.log('\n🔄 CONVERTING RUGS TO QUOTE MODE...\n');
    ...
    if (!category) {
      console.log('❌ RUGS category not found');
      return;
    }
    
    console.log(`📂 Working with RUGS Category: ${category.name} (${category._id})`);
    ...
    console.log(`📦 Found ${products.length} rugs products to convert`);
    
    if (products.length === 0) {
      console.log('✅ No rugs products to convert');
      return;
    }
    ...
    console.log(`   Total RUGS Products: ${convertedProducts.length}`);
    ...
    console.log('\n🎉 RUGS CONVERSION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error converting rugs to quote mode:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await convertRUGSToQuote();
