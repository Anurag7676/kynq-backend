...
const convertCOMMERCIALSEATINGToQuote = async () => {
  try {
    console.log('\n🔄 CONVERTING COMMERCIAL SEATING TO QUOTE MODE...\n');
    
    // Find commercial-seating category
    const category = await Category.findOne({ 
      $or: [
        { name: { $regex: /commercial-seating/i } },
        { slug: { $regex: /commercial-seating/i } }
      ]
    });
    
    if (!category) {
      console.log('❌ COMMERCIAL SEATING category not found');
      return;
    }
    
    console.log(`📂 Working with COMMERCIAL SEATING Category: ${category.name} (${category._id})`);
    ...
    console.log(`📦 Found ${products.length} commercial-seating products to convert`);
    
    if (products.length === 0) {
      console.log('✅ No commercial-seating products to convert');
      return;
    }
    ...
    console.log(`   Total COMMERCIAL SEATING Products: ${convertedProducts.length}`);
    ...
    console.log('\n🎉 COMMERCIAL SEATING CONVERSION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error converting commercial-seating to quote mode:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await convertCOMMERCIALSEATINGToQuote();
