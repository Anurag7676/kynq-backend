...
const convertPENDANTLIGHTSToQuote = async () => {
  try {
    console.log('\n🔄 CONVERTING PENDANT LIGHTS TO QUOTE MODE...\n');
    
    // Find pendant-lights category
    const category = await Category.findOne({ 
      $or: [
        { name: { $regex: /pendant-lights/i } },
        { slug: { $regex: /pendant-lights/i } }
      ]
    });
    
    if (!category) {
      console.log('❌ PENDANT LIGHTS category not found');
      return;
    }
    
    console.log(`📂 Working with PENDANT LIGHTS Category: ${category.name} (${category._id})`);
    ...
    console.log(`📦 Found ${products.length} pendant-lights products to convert`);
    
    if (products.length === 0) {
      console.log('✅ No pendant-lights products to convert');
      return;
    }
    ...
    console.log(`   Total PENDANT LIGHTS Products: ${convertedProducts.length}`);
    ...
    console.log('\n🎉 PENDANT LIGHTS CONVERSION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error converting pendant-lights to quote mode:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await convertPENDANTLIGHTSToQuote();
