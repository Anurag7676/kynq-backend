...
const convertSMARTTOILETSToQuote = async () => {
  try {
    console.log('\n🔄 CONVERTING SMART TOILETS TO QUOTE MODE...\n');
    
    // Find smart-toilets category
    const category = await Category.findOne({ 
      $or: [
        { name: { $regex: /smart-toilets/i } },
        { slug: { $regex: /smart-toilets/i } }
      ]
    });
    
    if (!category) {
      console.log('❌ SMART TOILETS category not found');
      return;
    }
    
    console.log(`📂 Working with SMART TOILETS Category: ${category.name} (${category._id})`);
    ...
    console.log(`📦 Found ${products.length} smart-toilets products to convert`);
    
    if (products.length === 0) {
      console.log('✅ No smart-toilets products to convert');
      return;
    }
    ...
    console.log(`   Total SMART TOILETS Products: ${convertedProducts.length}`);
    ...
    console.log('\n🎉 SMART TOILETS CONVERSION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error converting smart-toilets to quote mode:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await convertSMARTTOILETSToQuote();
