...
const restorePATTERNTILESToNormal = async () => {
  try {
    console.log('\n🔄 RESTORING PATTERN TILES TO NORMAL PRICING...\n');
    ...
    console.log(`   Total PATTERN TILES Products: ${restoredProducts.length}`);
    ...
    if (priceStats.length > 0) {
      ...
      console.log(`   Regular PATTERN TILES with Price: ${stats.count}`);
    ...
    console.log('\n🎉 PATTERN TILES RESTORATION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error restoring pattern-tiles to normal pricing:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await restorePATTERNTILESToNormal();
