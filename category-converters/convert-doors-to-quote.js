...
const convertDOORSToQuote = async () => {
  try {
    console.log('\n🔄 CONVERTING DOORS TO QUOTE MODE...\n');
    ...
    if (!category) {
      console.log('❌ DOORS category not found');
      return;
    }
    
    console.log(`📂 Working with DOORS Category: ${category.name} (${category._id})`);
    ...
    console.log(`📦 Found ${products.length} doors products to convert`);
    
    if (products.length === 0) {
      console.log('✅ No doors products to convert');
      return;
    }
    ...
    console.log(`   Total DOORS Products: ${convertedProducts.length}`);
    ...
    console.log('\n🎉 DOORS CONVERSION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error converting doors to quote mode:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await convertDOORSToQuote();
