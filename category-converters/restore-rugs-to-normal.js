...
const restoreRUGSToNormal = async () => {
  try {
    console.log('\n🔄 RESTORING RUGS TO NORMAL PRICING...\n');
    ...
    console.log(`   Total RUGS Products: ${restoredProducts.length}`);
    ...
    if (priceStats.length > 0) {
      ...
      console.log(`   Regular RUGS with Price: ${stats.count}`);
    ...
    console.log('\n🎉 RUGS RESTORATION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error restoring rugs to normal pricing:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await restoreRUGSToNormal();
