...
const restoreOutdoorFurnitureToNormal = async () => {
  try {
    console.log('\n🔄 RESTORING OUTDOOR FURNITURE TO NORMAL PRICING...\n');
    ...
    console.log(`   Total Outdoor Furniture Products: ${restoredProducts.length}`);
    ...
    if (priceStats.length > 0) {
      ...
      console.log(`   Regular Outdoor Furniture with Price: ${stats.count}`);
    ...
    console.log('\n🎉 OUTDOOR FURNITURE RESTORATION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error restoring outdoor-furniture to normal pricing:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await restoreOutdoorFurnitureToNormal();
