...
const restoreOutdoorlightsToNormal = async () => {
  try {
    console.log('\n🔄 RESTORING OUTDOOR LIGHTS TO NORMAL PRICING...\n');
    ...
    console.log(`   Total Outdoor lights Products: ${restoredProducts.length}`);
    ...
    if (priceStats.length > 0) {
      ...
      console.log(`   Regular Outdoor lights with Price: ${stats.count}`);
    ...
    console.log('\n🎉 OUTDOOR LIGHTS RESTORATION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error restoring outdoor-lights to normal pricing:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await restoreOutdoorlightsToNormal();
