...
const restoreSAUNAROOMToNormal = async () => {
  try {
    console.log('\n🔄 RESTORING SAUNA ROOM TO NORMAL PRICING...\n');
    ...
    console.log(`   Total SAUNA ROOM Products: ${restoredProducts.length}`);
    ...
    if (priceStats.length > 0) {
      ...
      console.log(`   Regular SAUNA ROOM with Price: ${stats.count}`);
    ...
    console.log('\n🎉 SAUNA ROOM RESTORATION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error restoring sauna-room to normal pricing:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await restoreSAUNAROOMToNormal();
