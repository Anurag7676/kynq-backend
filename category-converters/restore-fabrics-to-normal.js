...
const restoreFABRICSToNormal = async () => {
  try {
    console.log('\n🔄 RESTORING FABRICS TO NORMAL PRICING...\n');
    ...
    console.log(`   Total FABRICS Products: ${restoredProducts.length}`);
    ...
    if (priceStats.length > 0) {
      ...
      console.log(`   Regular FABRICS with Price: ${stats.count}`);
    ...
    console.log('\n🎉 FABRICS RESTORATION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error restoring fabrics to normal pricing:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await restoreFABRICSToNormal();
