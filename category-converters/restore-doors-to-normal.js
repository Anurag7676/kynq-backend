...
const restoreDOORSToNormal = async () => {
  try {
    console.log('\n🔄 RESTORING DOORS TO NORMAL PRICING...\n');
    ...
    console.log(`   Total DOORS Products: ${restoredProducts.length}`);
    ...
    if (priceStats.length > 0) {
      ...
      console.log(`   Regular DOORS with Price: ${stats.count}`);
    ...
    console.log('\n🎉 DOORS RESTORATION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error restoring doors to normal pricing:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await restoreDOORSToNormal();
