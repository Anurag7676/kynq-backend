...
const convertJACUZZIToQuote = async () => {
  try {
    console.log('\n🔄 CONVERTING JACUZZI TO QUOTE MODE...\n');
    
    // Find jacuzzi category
    const category = await Category.findOne({ 
      $or: [
        { name: { $regex: /jacuzzi/i } },
        { slug: { $regex: /jacuzzi/i } }
      ]
    });
    
    if (!category) {
      console.log('❌ JACUZZI category not found');
      return;
    }
    
    console.log(`📂 Working with JACUZZI Category: ${category.name} (${category._id})`);
    ...
    console.log(`📦 Found ${products.length} jacuzzi products to convert`);
    
    if (products.length === 0) {
      console.log('✅ No jacuzzi products to convert');
      return;
    }
    ...
    console.log(`   Total JACUZZI Products: ${convertedProducts.length}`);
    ...
    console.log('\n🎉 JACUZZI CONVERSION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error converting jacuzzi to quote mode:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await convertJACUZZIToQuote();
