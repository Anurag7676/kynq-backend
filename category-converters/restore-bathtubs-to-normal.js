...
const restoreBATHTUBSToNormal = async () => {
  try {
    console.log('\n🔄 RESTORING BATHTUBS TO NORMAL PRICING...\n');
    ...
    console.log(`   Total BATHTUBS Products: ${restoredProducts.length}`);
    ...
    if (priceStats.length > 0) {
      ...
      console.log(`   Regular BATHTUBS with Price: ${stats.count}`);
    ...
    console.log('\n🎉 BATHTUBS RESTORATION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error restoring bathtubs to normal pricing:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await restoreBATHTUBSToNormal();
