...
const restoreIndoorFurnitureToNormal = async () => {
  try {
    console.log('\n🔄 RESTORING INDOOR FURNITURE TO NORMAL PRICING...\n');
    ...
    console.log(`   Total Indoor Furniture Products: ${restoredProducts.length}`);
    ...
    if (priceStats.length > 0) {
      ...
      console.log(`   Regular Indoor Furniture with Price: ${stats.count}`);
    ...
    console.log('\n🎉 INDOOR FURNITURE RESTORATION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error restoring indoor-furniture to normal pricing:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await restoreIndoorFurnitureToNormal();
