...
const convertOutdoorFurnitureToQuote = async () => {
  try {
    console.log('\n🔄 CONVERTING OUTDOOR FURNITURE TO QUOTE MODE...\n');
    
    // Find outdoor-furniture category
    const category = await Category.findOne({ 
      $or: [
        { name: { $regex: /outdoor-furniture/i } },
        { slug: { $regex: /outdoor-furniture/i } }
      ]
    });
    
    if (!category) {
      console.log('❌ Outdoor Furniture category not found');
      return;
    }
    
    console.log(`📂 Working with Outdoor Furniture Category: ${category.name} (${category._id})`);
    ...
    console.log(`📦 Found ${products.length} outdoor-furniture products to convert`);
    
    if (products.length === 0) {
      console.log('✅ No outdoor-furniture products to convert');
      return;
    }
    ...
    console.log(`   Total Outdoor Furniture Products: ${convertedProducts.length}`);
    ...
    console.log('\n🎉 OUTDOOR FURNITURE CONVERSION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error converting outdoor-furniture to quote mode:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await convertOutdoorFurnitureToQuote();
