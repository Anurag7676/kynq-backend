...
const convertBATHTUBSToQuote = async () => {
  try {
    console.log('\n🔄 CONVERTING BATHTUBS TO QUOTE MODE...\n');
    
    // Find bathtubs category
    const category = await Category.findOne({ 
      $or: [
        { name: { $regex: /bathtubs/i } },
        { slug: { $regex: /bathtubs/i } }
      ]
    });
    
    if (!category) {
      console.log('❌ BATHTUBS category not found');
      return;
    }
    
    console.log(`📂 Working with BATHTUBS Category: ${category.name} (${category._id})`);
    ...
    console.log(`📦 Found ${products.length} bathtubs products to convert`);
    
    if (products.length === 0) {
      console.log('✅ No bathtubs products to convert');
      return;
    }
    ...
    console.log(`   Total BATHTUBS Products: ${convertedProducts.length}`);
    ...
    console.log('\n🎉 BATHTUBS CONVERSION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error converting bathtubs to quote mode:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await convertBATHTUBSToQuote();
