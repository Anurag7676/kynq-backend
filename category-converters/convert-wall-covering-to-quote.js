...
const convertWALLCOVERINGToQuote = async () => {
  try {
    console.log('\n🔄 CONVERTING WALL COVERING TO QUOTE MODE...\n');
    
    // Find wall-covering category
    const category = await Category.findOne({ 
      $or: [
        { name: { $regex: /wall-covering/i } },
        { slug: { $regex: /wall-covering/i } }
      ]
    });
    
    if (!category) {
      console.log('❌ WALL COVERING category not found');
      return;
    }
    
    console.log(`📂 Working with WALL COVERING Category: ${category.name} (${category._id})`);
    ...
    console.log(`📦 Found ${products.length} wall-covering products to convert`);
    
    if (products.length === 0) {
      console.log('✅ No wall-covering products to convert');
      return;
    }
    ...
    console.log(`   Total WALL COVERING Products: ${convertedProducts.length}`);
    ...
    console.log('\n🎉 WALL COVERING CONVERSION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error converting wall-covering to quote mode:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await convertWALLCOVERINGToQuote();
