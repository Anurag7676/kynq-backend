...
const convertSTAIRCASEToQuote = async () => {
  try {
    console.log('\n🔄 CONVERTING STAIRCASE TO QUOTE MODE...\n');
    
    // Find staircase category
    const category = await Category.findOne({ 
      $or: [
        { name: { $regex: /staircase/i } },
        { slug: { $regex: /staircase/i } }
      ]
    });
    
    if (!category) {
      console.log('❌ STAIRCASE category not found');
      return;
    }
    
    console.log(`📂 Working with STAIRCASE Category: ${category.name} (${category._id})`);
    ...
    console.log(`📦 Found ${products.length} staircase products to convert`);
    
    if (products.length === 0) {
      console.log('✅ No staircase products to convert');
      return;
    }
    ...
    console.log(`   Total STAIRCASE Products: ${convertedProducts.length}`);
    ...
    console.log('\n🎉 STAIRCASE CONVERSION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error converting staircase to quote mode:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await convertSTAIRCASEToQuote();
