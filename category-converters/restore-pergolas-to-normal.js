...
const restorePERGOLASToNormal = async () => {
  try {
    console.log('\n🔄 RESTORING PERGOLAS TO NORMAL PRICING...\n');
    ...
    console.log(`   Total PERGOLAS Products: ${restoredProducts.length}`);
    ...
    if (priceStats.length > 0) {
      ...
      console.log(`   Regular PERGOLAS with Price: ${stats.count}`);
    ...
    console.log('\n🎉 PERGOLAS RESTORATION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error restoring pergolas to normal pricing:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await restorePERGOLASToNormal();
