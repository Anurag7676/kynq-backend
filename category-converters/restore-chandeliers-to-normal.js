...
const restoreCHANDELIERSToNormal = async () => {
  try {
    console.log('\n🔄 RESTORING CHANDELIERS TO NORMAL PRICING...\n');
    ...
    console.log(`   Total CHANDELIERS Products: ${restoredProducts.length}`);
    ...
    if (priceStats.length > 0) {
      ...
      console.log(`   Regular CHANDELIERS with Price: ${stats.count}`);
    ...
    console.log('\n🎉 CHANDELIERS RESTORATION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error restoring chandeliers to normal pricing:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await restoreCHANDELIERSToNormal();
