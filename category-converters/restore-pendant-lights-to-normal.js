...
const restorePENDANTLIGHTSToNormal = async () => {
  try {
    console.log('\n🔄 RESTORING PENDANT LIGHTS TO NORMAL PRICING...\n');
    ...
    console.log(`   Total PENDANT LIGHTS Products: ${restoredProducts.length}`);
    ...
    if (priceStats.length > 0) {
      ...
      console.log(`   Regular PENDANT LIGHTS with Price: ${stats.count}`);
    ...
    console.log('\n🎉 PENDANT LIGHTS RESTORATION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error restoring pendant-lights to normal pricing:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await restorePENDANTLIGHTSToNormal();
