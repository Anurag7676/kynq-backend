...
const convertFLOORINGToQuote = async () => {
  try {
    console.log('\n🔄 CONVERTING FLOORING TO QUOTE MODE...\n');
    
    // Find flooring category
    const category = await Category.findOne({ 
      $or: [
        { name: { $regex: /flooring/i } },
        { slug: { $regex: /flooring/i } }
      ]
    });
    
    if (!category) {
      console.log('❌ FLOORING category not found');
      return;
    }
    
    console.log(`📂 Working with FLOORING Category: ${category.name} (${category._id})`);
    ...
    console.log(`📦 Found ${products.length} flooring products to convert`);
    
    if (products.length === 0) {
      console.log('✅ No flooring products to convert');
      return;
    }
    ...
    console.log(`   Total FLOORING Products: ${convertedProducts.length}`);
    ...
    console.log('\n🎉 FLOORING CONVERSION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error converting flooring to quote mode:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await convertFLOORINGToQuote();
