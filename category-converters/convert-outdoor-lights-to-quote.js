...
const convertOutdoorlightsToQuote = async () => {
  try {
    console.log('\n🔄 CONVERTING OUTDOOR LIGHTS TO QUOTE MODE...\n');
    
    // Find outdoor-lights category
    const category = await Category.findOne({ 
      $or: [
        { name: { $regex: /outdoor-lights/i } },
        { slug: { $regex: /outdoor-lights/i } }
      ]
    });
    
    if (!category) {
      console.log('❌ Outdoor lights category not found');
      return;
    }
    
    console.log(`📂 Working with Outdoor lights Category: ${category.name} (${category._id})`);
    ...
    console.log(`📦 Found ${products.length} outdoor-lights products to convert`);
    
    if (products.length === 0) {
      console.log('✅ No outdoor-lights products to convert');
      return;
    }
    ...
    console.log(`   Total Outdoor lights Products: ${convertedProducts.length}`);
    ...
    console.log('\n🎉 OUTDOOR LIGHTS CONVERSION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error converting outdoor-lights to quote mode:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await convertOutdoorlightsToQuote();
