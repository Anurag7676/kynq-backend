...
const restoreSTAIRCASEToNormal = async () => {
  try {
    console.log('\n🔄 RESTORING STAIRCASE TO NORMAL PRICING...\n');
    ...
    console.log(`   Total STAIRCASE Products: ${restoredProducts.length}`);
    ...
    if (priceStats.length > 0) {
      ...
      console.log(`   Regular STAIRCASE with Price: ${stats.count}`);
    ...
    console.log('\n🎉 STAIRCASE RESTORATION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error restoring staircase to normal pricing:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await restoreSTAIRCASEToNormal();
