...
const convertWallPanelsAndWallCoveringsToQuote = async () => {
  try {
    console.log('\n🔄 CONVERTING WALL PANELS & WALL COVERINGS TO QUOTE MODE...\n');
    
    // Find wall-panels-and-wall-coverings category
    const category = await Category.findOne({ 
      $or: [
        { name: { $regex: /wall-panels-and-wall-coverings/i } },
        { slug: { $regex: /wall-panels-and-wall-coverings/i } }
      ]
    });
    
    if (!category) {
      console.log('❌ WALL PANELS & WALL COVERINGS category not found');
      return;
    }
    
    console.log(`📂 Working with WALL PANELS & WALL COVERINGS Category: ${category.name} (${category._id})`);
    ...
    console.log(`📦 Found ${products.length} wall-panels-and-wall-coverings products to convert`);
    
    if (products.length === 0) {
      console.log('✅ No wall-panels-and-wall-coverings products to convert');
      return;
    }
    ...
    console.log(`   Total WALL PANELS & WALL COVERINGS Products: ${convertedProducts.length}`);
    ...
    console.log('\n🎉 WALL PANELS & WALL COVERINGS CONVERSION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error converting wall-panels-and-wall-coverings to quote mode:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await convertWallPanelsAndWallCoveringsToQuote();
