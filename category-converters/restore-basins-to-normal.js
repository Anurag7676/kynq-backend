...
const restoreBASINSToNormal = async () => {
  try {
    console.log('\n🔄 RESTORING BASINS TO NORMAL PRICING...\n');
    ...
    console.log(`   Total BASINS Products: ${restoredProducts.length}`);
    ...
    if (priceStats.length > 0) {
      ...
      console.log(`   Regular BASINS with Price: ${stats.count}`);
    ...
    console.log('\n🎉 BASINS RESTORATION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error restoring basins to normal pricing:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await restoreBASINSToNormal();
