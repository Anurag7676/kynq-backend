...
const convertCHANDELIERSToQuote = async () => {
  try {
    console.log('\n🔄 CONVERTING CHANDELIERS TO QUOTE MODE...\n');
    
    // Find chandeliers category
    const category = await Category.findOne({ 
      $or: [
        { name: { $regex: /chandeliers/i } },
        { slug: { $regex: /chandeliers/i } }
      ]
    });
    
    if (!category) {
      console.log('❌ CHANDELIERS category not found');
      return;
    }
    
    console.log(`📂 Working with CHANDELIERS Category: ${category.name} (${category._id})`);
    ...
    console.log(`📦 Found ${products.length} chandeliers products to convert`);
    
    if (products.length === 0) {
      console.log('✅ No chandeliers products to convert');
      return;
    }
    ...
    console.log(`   Total CHANDELIERS Products: ${convertedProducts.length}`);
    ...
    console.log('\n🎉 CHANDELIERS CONVERSION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error converting chandeliers to quote mode:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await convertCHANDELIERSToQuote();
