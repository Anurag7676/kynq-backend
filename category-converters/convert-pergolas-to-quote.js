...
const convertPERGOLASToQuote = async () => {
  try {
    console.log('\n🔄 CONVERTING PERGOLAS TO QUOTE MODE...\n');
    
    // Find pergolas category
    const category = await Category.findOne({ 
      $or: [
        { name: { $regex: /pergolas/i } },
        { slug: { $regex: /pergolas/i } }
      ]
    });
    
    if (!category) {
      console.log('❌ PERGOLAS category not found');
      return;
    }
    
    console.log(`📂 Working with PERGOLAS Category: ${category.name} (${category._id})`);
    ...
    console.log(`📦 Found ${products.length} pergolas products to convert`);
    
    if (products.length === 0) {
      console.log('✅ No pergolas products to convert');
      return;
    }
    ...
    console.log(`   Total PERGOLAS Products: ${convertedProducts.length}`);
    ...
    console.log('\n🎉 PERGOLAS CONVERSION COMPLETED SUCCESSFULLY!');
    console.log('\n📊 SUMMARY:');
    ...
    console.error('❌ Error converting pergolas to quote mode:', error);
    throw error;
  }
};
...
const main = async () => {
  try {
    await connectDB();
    const result = await convertPERGOLASToQuote();
